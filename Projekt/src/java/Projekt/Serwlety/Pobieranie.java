 ...
 * @author student
 */
@WebServlet(name = "Pobieranie", urlPatterns = {"/Pobieranie"})
public class Pobieranie extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<title>Download files</title>");                        
            out.println("</head>");
            out.println("<body>");
            ...
            String pobranyElement = request.getParameter("download_file_id");
            int file_id = Integer.parseInt(pobranyElement);
            
            
            
            if(pobranyElement.isEmpty()){
                out.println("<h3> PLEASE WRITE ID OF FILE YOU WANT TO DOWNLOAD  </h3><br><br>");
            }else{
                int czyIstnieje = new Hiber.PptQuery().walidacjaCzyIstaniejeId(file_id);
                if(czyIstnieje==0){
                    out.println("<h3> FILE DOWNLOADED </h3><br><br>");
                    PptQuery pliczek = new PptQuery();
                    pliczek.downloadFile(file_id);
            ...
            Logger.getLogger(Pobieranie.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
